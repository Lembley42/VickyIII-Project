...
namespace DigitalRuby.AdvancedPolygonCollider
{
    ...
    public class AdvancedPolygonCollider : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            UpdateDirtyState();
            if (dirty && spriteRenderer.sprite != null)
            {
        ...
        private List<Vector2[]> ProcessVertices(PolygonCollider2D collider, Vertices v, List<Vector2[]> list, ref PolygonParameters p, ref int pathIndex)
        {
            ...
            if (p.Decompose)
            {
                ...
                for (int j = 0; j < points.Count; j++)
                {
                    ...
                    {
                        float xValue = -1.0f + (2.0f * (v2[i].x / p.Rect.width));
                        float yValue = -1.0f + (2.0f * (v2[i].y / p.Rect.height));
                        v2[i] = new Vector2(xValue * p.XMultiplier, yValue * p.YMultiplier);
                    }
            ...
            else
            {
                ...
                {
                    float xValue = -1.0f + (2.0f * (v[i].x / p.Rect.width));
                    float yValue = -1.0f + (2.0f * (v[i].y / p.Rect.height));
                    v[i] = new Vector2(xValue * p.XMultiplier, yValue * p.YMultiplier);
                }
